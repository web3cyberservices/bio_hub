          ...
          <Tabs key={profileType} value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6 md:space-y-10">
            <div className="flex justify-center">
              <TabsList className="bg-white/60 backdrop-blur-md p-1 rounded-xl h-14 md:h-20 border shadow-md max-w-6xl w-full overflow-x-auto no-scrollbar">
                ...
                {profileType === 'user' ? (
                  <>
                    <TabsTrigger value="dashboard" className="rounded-lg px-4 font-black uppercase tracking-widest text-[7px] md:text-[10px] gap-2 data-[state=active]:bg-primary h-full flex-1"><LayoutDashboard className="h-4 w-4" /> Дашборд</TabsTrigger>
                    <TabsTrigger value="meals" className="rounded-lg px-4 font-black uppercase tracking-widest text-[7px] md:text-[10px] gap-2 data-[state=active]:bg-primary h-full flex-1"><Utensils className="h-4 w-4" /> Питание</TabsTrigger>
                  </>
                ) : (
                  <>
                    <TabsTrigger value="patients" className="rounded-lg px-4 font-black uppercase tracking-widest text-[7px] md:text-[10px] gap-2 data-[state=active]:bg-primary h-full flex-1"><Users className="h-4 w-4" /> Пациенты</TabsTrigger>
                    <TabsTrigger value="my-feed" className="rounded-lg px-4 font-black uppercase tracking-widest text-[7px] md:text-[10px] gap-2 data-[state=active]:bg-primary h-full flex-1"><Briefcase className="h-4 w-4" /> Посты</TabsTrigger>
                  </>
                )}
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <TabsContent value="my-feed" className="mt-0"><div className="max-w-4xl mx-auto space-y-6"><h2 className="text-3xl font-black font-headline tracking-tighter">Мои посты</h2>{posts?.filter(p => p.authorId === user.uid).map(post => (<Card key={post.id} className="premium-card p-8 space-y-4"><p className="text-lg font-medium">{post.content}</p></Card>))}</div></TabsContent>
