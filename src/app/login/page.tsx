...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Редирект только если это не гость и не идет загрузка
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Запрашиваем доступы к биометрии
      ...
      // Извлекаем и сохраняем токен для Google Fit
      const credential = GoogleAuthProvider.credentialFromResult(userCredential);
      if (credential?.accessToken) {
        sessionStorage.setItem('google_fit_token', credential.accessToken);
      }

      // Проверяем/Создаем документ пользователя
      ...
      toast({ 
        title: 'Вход через Google выполнен', 
        description: 'Ваш био-хаб готов к работе.' 
      });
    } catch (error: any) {
      ...
      if (error.code === 'auth/popup-closed-by-user') {
        ...
        errorMsg = 'Вход через Google не включен в настройках Firebase.';
      }
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center text-[9px] font-black uppercase tracking-widest">
                <span className="bg-[#0c1221] px-4 text-white/30">Или через email</span>
              </div>
            </div>
            ...
            <form onSubmit={handleEmailLogin} className="space-y-4">
              ...
              <Button 
                className="w-full h-14 bg-primary hover:bg-primary/90 rounded-xl font-black text-slate-950 shadow-xl shadow-primary/10" 
                type="submit"
                disabled={loading}
              >
                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <div className="flex items-center"><LogIn className="h-5 w-5 mr-2" /> ВОЙТИ В ХАБ</div>}
              </Button>
            </form>
