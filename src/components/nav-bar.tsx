  ...
  return (
    <nav className="sticky top-0 z-[100] w-full bg-primary/90 backdrop-blur-xl border-b border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.1)]">
      <div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-8">
        ...
        {/* Auth buttons: centered on mobile, right-aligned on desktop */}
        <div className="flex items-center gap-4 w-full justify-center md:w-auto md:justify-end">
          {!loading && (
            <>
              {user ? (
                <Link href="/dashboard">
                  <Button className="rounded-2xl px-6 md:px-8 h-12 font-black uppercase tracking-widest text-[10px] bg-white text-primary shadow-xl hover:bg-white/90 transition-transform active:scale-95 flex gap-2">
                    <LayoutDashboard className="h-4 w-4" /> В кабинет
                  </Button>
                </Link>
              ) : (
                <>
                  <Link href="/login">
                    <Button variant="ghost" className="h-12 font-black text-[10px] uppercase tracking-widest text-white hover:bg-white/10 px-4 md:px-6 flex gap-2">
                      <LogIn className="h-4 w-4" /> Войти
                    </Button>
                  </Link>
                  <Link href="/register">
                    <Button className="rounded-2xl px-6 md:px-8 h-12 font-black uppercase tracking-widest text-[10px] bg-white text-primary shadow-xl hover:bg-white/90 transition-transform active:scale-95">
                      Регистрация
                    </Button>
                  </Link>
                </>
