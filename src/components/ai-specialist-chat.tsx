...
export function AISpecialistChat({ onBack, className }: AISpecialistChatProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      // КРИТИЧЕСКИ: Динамический импорт внутри функции для предотвращения HMR ошибок Turbopack
    ...
    <div className={cn("flex flex-col h-full bg-transparent relative overflow-hidden", className)}>
      <div className="p-4 md:p-6 bg-primary/10 border-b border-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div><h3 className="text-white font-black text-sm md:text-base tracking-tight uppercase">ИИ-Специалист PRO</h3><div className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /><span className="text-[8px] md:text-[10px] font-bold text-primary/60 uppercase tracking-widest">Active Intelligence</span></div></div>
        </div>
      </div>
          ...
          {messages.map((m, i) => (
            <div key={i} className={cn("flex flex-col gap-2", m.role === 'user' ? "items-end" : "items-start")}>
              <div className={cn("p-4 md:p-6 rounded-[1.8rem] text-sm md:text-base font-medium shadow-lg transition-all relative max-w-[85%] md:max-w-[75%]", m.role === 'user' ? "bg-primary text-slate-950 rounded-tr-none" : "bg-white/5 text-white/90 rounded-tl-none border border-white/10 backdrop-blur-sm")}>
                <div className="flex items-center gap-2 mb-2 opacity-40">{m.role === 'user' ? <User className="h-3 w-3" /> : <Bot className="h-3 w-3" />}<span className="text-[8px] font-black uppercase tracking-widest">{m.role === 'user' ? 'Вы' : 'ИИ Помощник'}</span></div>
                {m.content}
              </div>
            </div>
          ))}
          {loading && <div className="flex flex-col items-start gap-2"><div className="bg-white/5 p-4 md:p-6 rounded-[1.8rem] rounded-tl-none border border-white/10 flex items-center gap-3"><Loader2 className="h-5 w-5 animate-spin text-primary" /><span className="text-xs font-black uppercase tracking-widest text-primary/40 animate-pulse">Генерация ответа...</span></div></div>}
          <div ref={scrollRef} />
        </div>
      </ScrollArea>
      <div className="p-4 md:p-8 bg-black/20 border-t border-white/5 shrink-0">
        <div className="relative flex items-center gap-4">
          ...
          <Button size="icon" onClick={handleSend} disabled={loading || !input.trim()} className="h-14 md:h-16 w-14 md:w-16 rounded-xl md:rounded-[2rem] bg-primary shadow-xl shadow-primary/20 shrink-0"><Send className="h-5 w-5 md:h-6 md:w-6 text-slate-950" /></Button>
        </div>
      </div>
    </div>
  );
