...
export function UnifiedDataEntry({ children, selectedDate = new Date() }: UnifiedDataEntryProps) {
  ...
  const handleSubmit = async (isRefinement = false) => {
    if (!description && !image && !steps && !heartRate && !sleep && !refinement && !water && !weight) {
    ...
    try {
      if (activeTab === 'meal') {
        ...
        if (isRefinement) {
          setRefinement('');
          toast({ title: 'ИИ обучен', description: 'Данные обновлены.' });
        }
      } else {
  ...
  const inputClasses = "h-14 rounded-xl bg-primary/10 border-none font-bold text-foreground text-lg placeholder:text-muted-foreground/40 focus:ring-4 focus:ring-primary/20 transition-all";
  const textareaClasses = "min-h-[150px] rounded-3xl bg-primary/5 border-none p-6 text-lg font-medium text-foreground resize-none placeholder:text-muted-foreground/40 focus:ring-4 focus:ring-primary/10 transition-all";
        ...
        <div className="p-8 space-y-6 max-h-[80vh] overflow-y-auto no-scrollbar">
          {!mealResult && !isSuccess ? (
            <>
              <Tabs defaultValue="meal" value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="grid w-full grid-cols-5 rounded-2xl h-14 bg-muted/50 p-1">
                  ...
                  <TabsTrigger value="metrics" className="rounded-xl font-bold gap-2 text-[10px]"><Scale className="h-4 w-4" /> Вес/Вода</TabsTrigger>
                ...
                <TabsContent value="metrics" className="mt-8 space-y-8 animate-in fade-in duration-300">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2"><Scale className="h-3 w-3" /> Вес (кг)</label>
                      <Input placeholder="75.0" value={weight} onChange={e => setWeight(e.target.value)} type="number" className={inputClasses} />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2"><Droplet className="h-3 w-3" /> Вода (мл)</label>
                ...
                <TabsContent value="activities" className="mt-8 space-y-6 animate-in fade-in duration-300">
                   <div className="grid grid-cols-3 gap-3">
                    {sports.map((sport) => (
                      <Button key={sport.name} variant="outline" className="h-20 rounded-2xl flex flex-col gap-1 border-2 border-muted hover:border-primary transition-all" onClick={() => addActivity(sport.name)}>
                        <sport.icon className="h-5 w-5 text-primary" />
                        <span className="text-xs font-bold">{sport.name}</span>
                      </Button>
                    ))}
                  </div>
                  <Textarea placeholder="Опишите ваши тренировки сегодня..." value={description} onChange={(e) => setDescription(e.target.value)} className={textareaClasses} />
                  <Button className="w-full h-20 rounded-[1.75rem] text-2xl font-black bg-primary shadow-xl shadow-primary/20" onClick={() => handleSubmit(false)}>Записать активность</Button>
                </TabsContent>
                ...
                <TabsContent value="devices" className="mt-8 space-y-8 animate-in fade-in duration-300">
                  ...
                  <Button className="w-full h-20 rounded-[1.75rem] text-2xl font-black bg-primary shadow-xl shadow-primary/20" onClick={() => handleSubmit(false)}>Синхронизировать</Button>
                </TabsContent>

                <TabsContent value="meal" className="mt-8 space-y-6">
                  <Textarea placeholder="Что вы съели? Опишите или добавьте фото..." value={description} onChange={(e) => setDescription(e.target.value)} className={textareaClasses} />
                  <div className="grid grid-cols-2 gap-4">
                    <Button variant="outline" className="h-28 rounded-[2rem] border-dashed border-2 flex flex-col gap-2 hover:bg-primary/5 transition-all" onClick={startCamera}>
                  ...
                  {showCamera && (
                    <div className="relative rounded-[2.5rem] overflow-hidden bg-black aspect-video shadow-2xl">
                      ...
                      <div className="absolute bottom-6 left-0 right-0 flex justify-center gap-4">
                        <Button onClick={capturePhoto} className="rounded-full w-14 h-14 bg-white text-primary hover:scale-110 transition-all"><Camera className="h-7 w-7" /></Button>
                  ...
                  <Button className="w-full h-20 rounded-[1.75rem] text-2xl font-black bg-primary shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all" onClick={() => handleSubmit(false)} disabled={loading}>
                ...
                <TabsContent value="labs" className="mt-8 space-y-6">
                   <Textarea placeholder="Введите данные анализов (Гемоглобин, Ферритин и др.)..." value={description} onChange={(e) => setDescription(e.target.value)} className={textareaClasses} />
            ...
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="bg-muted/30 p-6 rounded-[2rem] border-l-4 border-primary shadow-inner">
              ...
              <Button className="w-full h-18 rounded-2xl font-black text-xl bg-muted text-foreground hover:bg-muted/80 transition-all" onClick={reset}>Вернуться назад</Button>
            </div>
          ) : (
            <div className="py-20 flex flex-col items-center text-center space-y-6 animate-in zoom-in duration-500">
              ...
              <p className="text-muted-foreground font-medium max-w-xs">Биометрический хаб получил ваши данные. Рекомендации обновляются.</p>
              <Button className="w-64 h-16 rounded-2xl font-bold text-lg bg-primary shadow-xl shadow-primary/20" onClick={reset}>Понятно</Button>
            </div>
          )}
        </div>
      </DialogContent>
