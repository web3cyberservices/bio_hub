...
const NeonGauge = ({ label, value, icon, color, progress, className }: GaugeProps) => {
  return (
    <div className={cn("flex flex-col items-center justify-center gap-1.5 md:gap-2 group transition-all duration-500", className)}>
      <div className="text-white/40 group-hover:text-white transition-colors scale-75 md:scale-100">
        {icon}
      </div>
      <div className="relative w-14 h-14 md:w-24 md:h-24 flex items-center justify-center">
        ...
        <div className="text-center">
          <span className="text-xs md:text-xl font-black text-white leading-none block drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">{value}</span>
        </div>
      </div>
      <span className="text-[7px] md:text-[9px] font-black uppercase text-white/30 tracking-[0.2em] group-hover:text-primary transition-colors">
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div className={cn("relative w-full h-full flex items-center justify-center overflow-hidden bg-[#000000] touch-none", className)}>
      ...
      <div className="relative z-10 w-full max-w-7xl h-full flex items-center justify-between px-4 md:px-12">
        ...
        {/* LEFT HUD */}
        <div className="flex flex-col gap-12 md:gap-24 items-start justify-center h-full">
          <NeonGauge 
        ...
        {/* RIGHT HUD */}
        <div className="flex flex-col gap-12 md:gap-24 items-end justify-center h-full">
          <NeonGauge 
      ...
      <div className="absolute bottom-32 left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-24 z-20 flex items-center gap-3 bg-white/5 border border-white/10 px-5 py-2 md:px-6 md:py-3 rounded-2xl backdrop-blur-md shadow-2xl">
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-primary/10 flex items-center justify-center">
          <Footprints className="h-4 w-4 md:h-5 md:w-5 text-[#00ffff]" />
        </div>
        <div className="flex flex-col">
          <span className="text-lg md:text-xl font-black text-white leading-none">{stepsVal}</span>
          <span className="text-[7px] md:text-[9px] font-black text-white/30 uppercase tracking-widest mt-0.5 whitespace-nowrap">STEPS</span>
        </div>
      </div>

      {/* LAYER 4: BIO-CORE HEART BEAT */}
      <div className="absolute top-[40%] md:top-[36%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none">
        <div className="relative w-10 h-10 md:w-12 md:h-12 flex items-center justify-center">
          <div className="absolute inset-0 bg-[#00ffff]/30 rounded-full animate-ping opacity-60" />
          <div className="w-3 h-3 md:w-4 md:h-4 rounded-full bg-[#00ffff] shadow-[0_0_25px_#00ffff] animate-pulse" />
        </div>
      </div>

      {/* LAYER 5: THE PERSON (TOP LAYER) */}
      <div className="absolute inset-0 z-50 pointer-events-none flex items-center justify-center">
        <div className="relative w-[120vw] h-[65vh] md:w-full md:h-[68vh] max-w-none md:max-w-6xl animate-hologram flex items-center justify-center transition-all duration-700 -translate-y-2 md:-translate-y-14">
          <Image 
