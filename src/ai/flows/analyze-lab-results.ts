...
const AnalyzeLabOutputSchema = z.object({
  ...
  markers: z.array(z.object({
    ...
    referenceRange: z.string().describe('Норма или референсный интервал. Если его нет в документе, предложи экспертную норму для пола и возраста пользователя.'),
...
3. ВАЖНО: Если в документе ОТСУТСТВУЕТ колонка "Норма" или она пуста, вы ДОЛЖНЫ предоставить экспертную норму, основываясь на данных профиля пользователя (пол и возраст) и стандартах доказательной медицины.
4. Оцените статус каждого маркера (normal/high/low) на основе найденных или предложенных вами норм.
5. Результат должен быть СТРОГО на русском языке.
6. НИКОГДА не возвращайте "undefined" или "null" в текстовых полях. Если норма неизвестна, напишите "По возрасту" или примерный интервал.
