...
1. Поле referenceRange должно содержать ТОЛЬКО ЧИСЛОВОЙ ИНТЕРВАЛ (например: "4.1 — 5.9").
2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать фразы: "не указано", "норма не указана", "N/A", "undefined", "см. бланк", "по возрасту", "зависит от...".
3. Если в документе колонка нормы пуста, вы ДОЛЖНЫ предоставить ЭКСПЕРТНУЮ МЕДИЦИНСКУЮ НОРМУ для указанного пола и возраста пользователя.
4. Вы несете ответственность за точность цифр. Используйте актуальные международные стандарты (ВОЗ, справочники).

ИНСТРУКЦИЯ:
- Тщательно распознавайте значения. 
- Если показатель отклонен, в interpretation кратко напишите почему это важно.
