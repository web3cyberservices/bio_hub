...
const AnalyzeLabOutputSchema = z.object({
  ...
  markers: z.array(z.object({
    ...
    referenceRange: z.string().describe('Норма или референсный интервал, указанный в документе (например, "4.1 - 5.9").'),
...
4. ОБЯЗАТЕЛЬНО извлеките норму в поле referenceRange.
5. В поле interpretation дайте краткий медицинский комментарий.
6. Результат должен быть СТРОГО на русском языке.
7. ВАЖНО: Модель должна прочитать каждую цифру максимально внимательно. Если данные на фото неразборчивы, попробуйте восстановить их по контексту таблицы.
