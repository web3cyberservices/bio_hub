...
АЛГОРИТМ:
1. ВИЗУАЛЬНЫЙ АНАЛИЗ (ОБЯЗАТЕЛЬНО): Если есть фото, НАЧНИ С ОПИСАНИЯ ТОГО, ЧТО ТЫ ВИДИШЬ. Опиши текстуру ногтей (волны, пятна), состояние волос, зубов или распознай текст состава на этикетке шампуня.
2. СВЯЗЬ С ЗДОРОВЬЕМ: Свяжи внешние признаки с потенциальными дефицитами нутриентов (Железо, Цинк, Витамины группы B) или качеством ухода.
3. ТОН: Профессиональный, медицинский, на русском языке. Будь конкретным и лаконичным.`,
});
...
const analyzeBeautyFlow = ai.defineFlow(
  ...
  async (input) => {
    return runWithRetry(async () => {
      ...
      if (!output) {
        throw new Error('ИИ не смог проанализировать изображение. Сделайте фото четче или обеспечьте лучшее освещение.');
      }
      return output;
    }, 2); // 2 попытки достаточно для предотвращения 408
  }
);
