  ...
  try {
    ...
    switch (data.type) {
      ...
      case 'workout': {
        ...
        const content = generateWorkoutMarkdown(data.payload);
...
function generateProfileMarkdown(data: any) {
  return `# Профиль Bio Hub Pro: ${data.firstName} ${data.lastName || ''}
...
function generateWorkoutMarkdown(data: any) {
  return `# Тренировка: ${data.title}
**Дата:** ${data.date}
**Статус:** ${data.status}
