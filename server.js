...
app.post('/api/run/:method', (req, res) => {
  ...
  if (method === 'nuclei') command = `nuclei -u ${target} -o reports/${scan_id}.json`;
  if (method === 'full-recon') command = `subfinder -d ${target} | naabu | httpx -o reports/${scan_id}.txt`;
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', engine: 'v2.4.0-stable' });
});
